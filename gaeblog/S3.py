...
class Connection:
    ...
    def put_acl(self, bucket, key, acl_xml_document, headers={}):
        return Response(
                self._make_request('PUT', bucket, key, {'acl': None},
...
class QueryStringAuthGenerator:
    ...
    def put(self, bucket, key, object, headers={}):
        ...
        return self.generate_url('PUT', bucket, key, {},
...
class S3Error():
    ''' represents an error response from S3 '''
    def __init__(self, code="", message="", key="", request_id=""):
        self.code = code
...
class ObjectBase(xml.sax.ContentHandler):
    ''' provide some basic xml response parsing '''
    error = None
    tag_stack = []
    ...
    def characters(self, content):
        xpath = '/'.join(self.tag_stack)
        if xpath == 'Error/Code':
            self.error.code += content
        elif xpath == 'Error/Message':
            self.error.message += content
        elif xpath == 'Error/RequestId':
            self.error.request_id += content
        elif xpath == 'Error/Key':
            self.error.key += content


class S3Object(ObjectBase):
    ''' modified to extend objectbase, which parses xml '''
    def __init__(self, data, metadata={}, parse_xml=False):
        self.data = data
...
class GetResponse(Response):
    def __init__(self, http_response):
        Response.__init__(self, http_response)
        response_headers = http_response.headers  # msg   # older pythons don't have getheaders
